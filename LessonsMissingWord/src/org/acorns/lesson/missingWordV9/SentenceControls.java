...
;			@Override
   ...
   private class ToolTipButton extends JButton
   {
    ...
    {   
   	...
   	{
   		char[][][] specials = LessonsMissingWord.SPECIALS;
    	for (char[][] row: specials)
    	{
    		String regex, replacement, hex1, hex2;
    		for (char[] column: row)
    		{
    			replacement = "" + ((column[0]==0)? "" : column[0]);
    			hex1 = "000" + Integer.toHexString(column[1]);
    			hex2 = "000" + Integer.toHexString(column[2]);
    			hex1 = hex1.substring(hex1.length()-4);
    			hex2 = hex2.substring(hex2.length()-4);
    			hex1 = "\\u" + hex1;
    			hex2 = "\\u" + hex2;
    			regex = "[" + hex1 + "-" + hex2 + "]";
